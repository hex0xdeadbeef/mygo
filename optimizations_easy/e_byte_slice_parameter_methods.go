...
2. A good example of comparison: time.Format and time.AppendFormat. The first one returns a string (under the hood it a;;pcates byte slice and call the function time.AppendFormat
	over it). The second one takes byte slice, writes formatted time representation and returns expended byte slice.
