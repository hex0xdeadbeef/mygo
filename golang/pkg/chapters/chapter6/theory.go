...
because each call to the method results in a creation of parameters copies.
...
7. Since each type has its own name space for method, we can use the name Distance for other methods so long, as
they belong to different types.

8. WE CAN CREATE NAMED SLICE TYPES AS PATH type Path []Point AND BIND TO THEM SOME METHODS
...
1. Instead of creation the copies of method parameters, we can declare an extra parameter as a pointer of its type.
This approach let us save the memory and change the caller object fields.
...
4. In order to use methods with an extra pointer receiver we should call this methods for an pointer type. There are
three options to do that:
	EXPLICIT

	1) Work with an object with type *Pointer
					- - - -
...
3. IF A FUNCTION HAS A POINTER PARAMETER AND WE CALL IT WITH AN ARGUMENT AND SUBSEQUENTLY CHANGE THE VALUE WITH THIS
LINK AND FINALLY ASSIGN TO THE LOCAL LINK NIL/OTHER VALUE, WE JUST CHANGE A DIRECTION THE POINTER SE TO NOT THE INIT
IAL VALUE.
...
a receiver of type ColoredPount.

2. The embedded anonymous fields promote theirs accessible methods to struct type they're embedded into.
...
receiver value. In other words we can extract a function and assign it to a variable and subsequetly call when we need it.

2. If we extract the method value and the method's receiver isn't a pointer, any object changes won't be reflected on the
result of the extracted method call. When we extract the method value, it saves a snapshot of the object.
...
6.6 ENCAPSULATION--------------------------------------------------------------------------------------------------------------------
1. If variable or method is inaccessible to a client of the object, it's said that it's encapsulated.

2. Encapsulation is sometimes calld "information hiding".
