...
2. The `state` consists of two parts (the size of each part is 32 bits):
	...
	2) Waiter (low 32 bits):
		This tracks the number of goroutines currently waiting for that counter (the high 32 bits) to hit zero. Every time we call wg.Wait(), it increases this "waiter" count. Once the `Counter` part reaches zero, it releases all the goroutines that were waiting.
...
1. The core of WaitGroup (the counter, the waiter, and semaphore) hasn't really changed across different Go versions. However, the way these elements are strucured has been modified many times.
	...
	For example, on a 64-bit system, a 64-bit value like uint64 should be ideally be stored at a memory address that's a multiple of 8 bytes. The reason is, the CPU can grab aligned data in one go, but if the data isn't aligned, it might take multiple ops to access it.
...
1. In Go 1.19 a key optimization was introduced to ensure that uint64 values used in atomic ops are always aligned to 8-byte boundaries, even on 32-bit architectures.
	...
	There's the magic happens.
		The align64 field itself doesn't take up any space, but it acts as a "marker" that tells Go compiler to handle the struct differently. The Go compiler recognizes align64 and automatically adjusts the struct's memory layout with the necessary padding, making sure that the entire struct starts at an 8-byte boundary.
...
1. Why don't we just split the `Counter` and `Waiter` into two separate `uint32` variables?
	...
	However, there's a trade-off. Using locks, like a mutex, adds overhead, especially when operations are happening frequently. Every time we lock and unlock, we're adding a bit of delay, which can stack up when dealing with high-frequency ops.
...
2. wg.Add(delta int)
	...
	However, if the negative delta causes the counter to drop below zero, the program will panic. The WaitGroup doesn't check the counter before updating it, it updates first and then checks. This means that if the counter goes negative after calling wg.Add(?), it stays negative until panic occurs.
3. The internals of wg.Add():
	...
	So, if we're reusing a WaitGroup, or if we want to wait for one batch of tasks, reset, and then wait for another batch, we need to make sure all calls to wg.Wait() are completed before starting new wg.Add(positive) calls for the next around of tasks.
...
4. wg.Wait()
	...
	If the CAS op fails, it means that another goroutine has modified the state, maybe the counter reached zero or it was incremented / decremented. In that case, wg.Wait() can't just assume everything is as it was, so it retries.
